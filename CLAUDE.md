...
- **Gotcha:** table `200` must never be empty. The kernel deletes its default route whenever the physical interface goes down (Wi-Fi drop/reconnect), and an empty table makes the lookup *fall through* to wg-quick's `table 51820` → `pia` instead of failing. With a `novpn` app running, this caused a self-sustaining encryption loop (WireGuard's encrypted outer packets apparently get re-marked `0x200` and fall into `pia` again), pinning `kworker/wg-crypt-pia` at 100% CPU. Two guards: `setup()` adds an interface-independent `unreachable default metric 4000` backstop to table 200 (fail closed), and `watch()`'s `ensure_route()` re-detects the physical iface/gateway every poll and restores the real default route (moving MASQUERADE if the interface changed).
...
- Self-healing watchdog: `pia-split-tunnel-watch.service` runs `pia-split-tunnel.sh watch` (ip rule, table 200 route, mangle mark, MASQUERADE, killswitch exception, novpn audio ACL).
